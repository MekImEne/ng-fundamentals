...
export class EventService {
  ...
  getEvents(): Observable<Ievent[]>{
    ...
    return subject; */
    return this.http.get<Ievent[]>('/api/events').pipe(catchError(
      this.handleError<Ievent[]>('getEvents', [])
    ));
  }

  getEvent(id: number): Observable<Ievent>{
    return this.http.get<Ievent>('/api/events/' + id).pipe(catchError(
  ...
  searchSessions(searchTerm: string): Observable<ISession[]>{
    return this.http.get<ISession[]>('/api/sessions/search?search=' + searchTerm).pipe(catchError(
...
const EVENTS: Ievent[] = [
  ...
  },
  {
    ...
    sessions: [
      ...
      {
        id: 6,
        name: 'These arent the directives you re looking for',
        presenter: 'John Papa',
        duration: 2,
        level: 'Intermediate',
